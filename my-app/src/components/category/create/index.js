...
import Navbar from "../../navbar"; // Додавання Navbar
...
const CategoryCreatePage = () => {
    ...
    const handlerOnSubmit = (e) => {
        e.preventDefault();
        // Використовуємо FormData для відправки форми з файлом
        const formData = new FormData();
        formData.append("name", data.name);
        formData.append("description", data.description);
        formData.append("imageFile", data.imageFile);

        axios.post(`${APP_ENV.URL}api/categories`, formData, {
            headers: { "Content-Type": "multipart/form-data" }
        })
            .then(() => {
                navigate("/"); // Перенаправлення після успішного додавання
            });
    }

    return (
        <>
            <Navbar /> {/* Додавання Navbar */}
